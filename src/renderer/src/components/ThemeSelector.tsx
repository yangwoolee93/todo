 ...
 * 밝은/어두운/자동 테마 선택 UI
 */
export function ThemeSelector({ mode, onChange }: ThemeSelectorProps) {
  return (
    <div className="flex flex-col gap-2">
      {OPTIONS.map((option) => (
        <label
          key={option.value}
          className={`flex cursor-pointer items-start gap-3 rounded-[var(--radius-btn)] border px-3 py-2.5 transition-colors ${
            mode === option.value
